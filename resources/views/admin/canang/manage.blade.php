            ...
            <div class="portlet light portlet-fit portlet-datatable bordered">
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <span class="caption-subject font-green sbold uppercase">Kelola Canang</span>
                    </div>
                    <div class="actions">
                        <div class="btn-group btn-group-devided">
                            <a href="{{ route('admin.canang.create') }}" class="btn btn-circle green">
                    ...
                    <div class="table-container">
                        <table class="table table-striped table-bordered table-hover" id="sample_3">
                            ...
                            @foreach($model as $row)
                                <tr>
                                    <td> {{ $no }}</td>
                                    <td> <img src="{{ url('assets/img/paket/'.$row->image) }}" style="width: 150px; height: 100px;"> </td>
                                    <td> {{ $row->nama_paket }}</td>
                                    <td> {{ number_format($row->harga,0,',','.') }}</td>
                                    <td> {{ $row->getStatus() }}</td>
                                    <td class="center" width="100">
                                        <a href="{{ route('admin.canang.detail',$row->id) }}" class="btn green btn-xs"><i class="fa fa-eye"></i></a>
                                        <a href="{{ route('admin.canang.edit',$row->id) }}" class="btn yellow-saffron btn-xs"><i class="fa fa-pencil"></i></a>
                                    </td>
                                </tr>
                                <?php $no++ ?>
                            @endforeach
